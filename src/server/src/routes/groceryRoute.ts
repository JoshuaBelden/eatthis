  ...
  public configure(app: express.Application): void {
    ...
    app
      ...
      .post(async (request, response) => {
        ...
        const result = await this.groceryController.createGroceryItemAsync(authResult.value.id, request.params.groceryId, request.body.line)
        return result.success
          ? response.status(201).send(result.value)
          : response.status(500).send(result.error)
      })
    ...
    app
      ...
      .delete(async (request, response) => {
        ...
        const result = await this.groceryController.deleteGroceryItemAsync(authResult.value.id, request.params.groceryId, request.params.groceryItemId)
        return result.success
          ? response.status(204).send(result.value)
          : response.status(500).send(result.error)
      })
